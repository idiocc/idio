...
function ja(a) {
  ...
  if ("number" == c && isFinite(a)) {
    return b.S ? (b = Math.abs(a), a = 864E5 <= b ? A(a, b, 864E5, "day") : 36E5 <= b ? A(a, b, 36E5, "hour") : 6E4 <= b ? A(a, b, 6E4, "minute") : 1000 <= b ? A(a, b, 1000, "second") : a + " ms") : (b = Math.abs(a), a = 864E5 <= b ? Math.round(a / 864E5) + "d" : 36E5 <= b ? Math.round(a / 36E5) + "h" : 6E4 <= b ? Math.round(a / 6E4) + "m" : 1000 <= b ? Math.round(a / 1000) + "s" : a + "ms"), a;
  }
...
function C(a, b) {
  ...
  const c = Math.abs(a), d = b && b.U || "", e = b && b.W || "", f = b && void 0 !== b.D ? b.D : 2, g = !(!b || !b.R);
  (b = b && b.V || "") && B[b.toLowerCase()] || (b = c >= B.pb ? "PB" : c >= B.tb ? "TB" : c >= B.gb ? "GB" : c >= B.mb ? "MB" : c >= B.kb ? "KB" : "B");
...
*/
let Ca = [0, 1996959894, 3993919788, 2567524794, 124634137, 1886057615, 3915621685, 2657392035, 249268274, 2044508324, 3772115230, 2547177864, 162941995, 2125561021, 3887607047, 2428444049, 498536548, 1789927666, 4089016648, 2227061214, 450548861, 1843258603, 4107580753, 2211677639, 325883990, 1684777152, 4251122042, 2321926636, 335633487, 1661365465, 4195302755, 2366115317, 997073096, 1281953886, 3579855332, 2724688242, 1006888145, 1258607687, 3524101629, 2768942443, 901097722, 1119000684, 3686517206, 
...
"undefined" !== typeof Int32Array && (Ca = new Int32Array(Ca));
...
;const K = G("koa-session:context");
async function Ga(a) {
  K("init from external");
  var b = a.ctx, c = a.opts;
  c.externalKey ? (b = c.externalKey.get(b), K("get external key from custom %s", b)) : (b = b.cookies.get(c.key, c), K("get external key from cookie %s", b));
  b ? (c = await a.store.get(b, c.maxAge, {rolling:c.rolling}), a.valid(c, b) ? (a.create(c, b), a.a = Da(JSON.stringify(a.session.toJSON()))) : a.create()) : a.create();
}
function Ha(a) {
  const b = a.a;
  var c = a.session;
  ...
  const d = c.toJSON();
  return b || Object.keys(d).length ? b !== Da(JSON.stringify(d)) ? "changed" : a.opts.rolling ? "rolling" : a.opts.renew && (a = c._expire, c = c.maxAge, a && c && a - Date.now() < c / 2) ? "renew" : "" : "";
}
class Ia {
  constructor(a, b = {}) {
    this.ctx = a;
    this.app = a.app;
    this.opts = b;
    this.store = this.opts.ContextStore ? new this.opts.ContextStore(a) : this.opts.store;
    ...
    if (!this.store) {
      a: {
        ...
        if (d) {
          ...
          K("parsed %j", b);
          this.valid(b) ? (this.create(b), this.a = Da(JSON.stringify(this.session.toJSON()))) : this.create();
        } else {
          this.create();
        }
      }
  ...
  valid(a, b) {
    ...
    if (a._expire && a._expire < Date.now()) {
      return K("expired session"), this.emit("expired", {key:b, value:a, ctx:c}), !1;
    }
    const d = this.opts.valid;
    return "function" !== typeof d || d(c, a) ? !0 : (K("invalid session"), this.emit("invalid", {key:b, value:a, ctx:c}), !1);
  }
  emit(a, b) {
    setImmediate(() => {
  ...
  create(a, b) {
    K("create session with val: %j externalKey: %s", a, b);
  ...
  async commit() {
    ...
    if (void 0 !== a) {
      if (!1 === a) {
        ...
        var d = Ha(this);
        K("should save session: %s", d);
        d && ("function" == typeof b && (K("before save"), b(c, a)), await this.save("changed" == d));
      }
    }
  }
  async remove() {
  ...
  async save(a) {
    const {opts:{key:b, rolling:c, encode:d, externalKey:e}, externalKey:f} = this;
    ...
    f ? (K("save %j to external key %s", h, f), "number" === typeof g && (g += 10000), await this.store.set(f, h, g, {changed:a, rolling:c}), e ? e.set(this.ctx, f) : this.ctx.cookies.set(b, f, this.opts)) : (K("save %j to cookie", h), h = d(h), K("save %s", h), this.ctx.cookies.set(b, h, this.opts));
  }
}
;/*
...
function La(a = {}) {
  ...
  Ja("session options %j", a);
  "function" != typeof a.encode && (a.encode = Fa);
  "function" != typeof a.decode && (a.decode = Ea);
  var b = a.store;
...
function Ma(a, b) {
  a.hasOwnProperty("CONTEXT_SESSION") || Object.defineProperties(a, {CONTEXT_SESSION:{get() {
    return this._CONTEXT_SESSION ? this._CONTEXT_SESSION : this._CONTEXT_SESSION = new Ia(this, b);
  }}, session:{get() {
...
class Pa {
  ...
  sign(a) {
    return Na(a, this.a, this.keys[0], this.encoding);
  }
  verify(a, b) {
  ...
  index(a, b) {
    for (let c = 0, d = this.keys.length; c < d; c++) {
      const e = Na(a, this.a, this.keys[c], this.encoding);
      if (Oa(b, e)) {
        return c;
      }
    }
  ...
  return async function(k, m) {
    ...
    if ("OPTIONS" != k.method) {
      ...
      try {
        ...
        throw k = n.headers || {}, m = Ra(k.vary || k.Vary || "", "Origin"), delete k.M, n.headers = Object.assign({}, k, l, {vary:m}), n;
      }
    } else {
...
;const Ua = path.basename, L = path.extname, Va = path.isAbsolute, Wa = path.join, Xa = path.normalize, Ya = path.parse, Za = path.resolve, $a = path.sep;
...
;const sb = async(...a) => await pb(cb, ...a), tb = async(...a) => await pb(db, ...a), ub = G("koa-send");
async function vb(a, b, c = {}) {
  ...
  if (e) {
    ...
    a.type || (h = b, h = "" !== d ? L(Ua(h, d)) : L(h), a.type = h);
    a.body = bb(b);
    return b;
  }
}
;const wb = G("koa-static");
var xb = (a, b = {}) => {
...
const Eb = /(?:\.0*|(\.[^0]+)0+)$/, N = {P:1, I:1024, J:1048576, H:1073741824, L:Math.pow(1024, 4), K:Math.pow(1024, 5)};
...
function Gb(a) {
  if ("string" == typeof a) {
    ...
    if ("number" == typeof a) {
      if (Number.isFinite(a)) {
        var c = Math.abs(a);
        b = "", b = c >= N.K ? "PB" : c >= N.L ? "TB" : c >= N.H ? "GB" : c >= N.J ? "MB" : c >= N.I ? "KB" : "B";
...
var Ib = (a = {}) => {
  ...
  return async function(d, e) {
    ...
    if (e && !d.res.headersSent && d.writable && !1 !== d.compress && "HEAD" !== d.request.method && !t[d.response.status] && !d.response.get("Content-Encoding") && (!0 === d.compress || b(d.response.type))) {
      ...
      "identity" != f && (Cb(e) && (e = d.body = JSON.stringify(e)), c && d.response.length < c || (d.set("Content-Encoding", f), d.res.removeHeader("Content-Length"), d = d.body = Hb[f](a), e instanceof M ? e.pipe(d) : d.end(e)));
    }
  };
};
const Jb = {["static"](a, b, {root:c = [], maxage:d, mount:e}) {
  ...
  if (!(c instanceof Pa || Array.isArray(c))) {
...
function Xb(a, b) {
  ...
  function d() {
    for (var n, q = 0; q < f.length; q++) {
      n = f[q], n.F.removeListener(n.event, n.G);
    }
  }
  function e(n) {
    b = n;
...
*/
const P = require("mime-db"), dc = /^\s*([^;\s]*)(?:;|\s|$)/, ec = /^text\//i, fc = Object.create(null), Q = Object.create(null);
gc();
function hc(a) {
...
class Lc {
  constructor(a, b, c) {
    ...
    this.request = a;
    this.a = b;
    c && (this.keys = Array.isArray(c.keys) ? new Pa(c.keys) : c.keys, this.secure = c.secure);
  }
  get(a, b) {
    const c = `${a}.sig`;
    var d;
...
;const xd = url.URL, T = url.Url, yd = url.format, zd = url.parse;
const Ad = net.isIP;
...
class Hd {
  ...
  get cookies() {
    this[W] || (this[W] = new Lc(this.req, this.res, {keys:this.app.keys, secure:this.request.secure}));
    return this[W];
  }
  set cookies(a) {
    this[W] = a;
...
S(S((new vd(Hd.prototype, "response")).method("attachment").method("redirect").method("remove").method("vary").method("set").method("append").method("flushHeaders").access("status").access("message").access("body").access("length").access("type").access("lastModified").access("etag"), "headerSent"), "writable");
S(S(S(S(S(S(S(S(S(S(S(S(S(S((new vd(Hd.prototype, "request")).method("acceptsLanguages").method("acceptsEncodings").method("acceptsCharsets").method("accepts").method("get").method("is").access("querystring").access("idempotent").access("socket").access("search").access("method").access("query").access("path").access("url").access("accept"), "origin"), "href"), "subdomains"), "protocol"), "host"), "hostname"), "URL"), "header"), "headers"), "secure"), "stale"), "fresh"), "ips"), "ip");
class Id {
  constructor() {
        ...
        if (b = -1 == a.indexOf("/") ? hc(a) : a) {
          if (!b.includes("charset")) {
            ...
            if (b && "string" == typeof b) {
              var d = (c = dc.exec(b)) && P[c[1].toLowerCase()];
...
;const Jd = G("@goa/koa:application");
async function Kd(a, b) {
  const c = a.res;
  c.statusCode = 404;
  Zb(c, d => a.onerror(d));
  try {
    return await b(a), Ld(a);
  } catch (d) {
    a.onerror(d);
  }
}
class Md extends Wb {
  constructor() {
    ...
    this.context = Object.create(Hd.prototype);
    this.request = Object.create(Gd.prototype);
    this.response = Object.create(Id.prototype);
    this.keys = void 0;
  }
  [z.custom]() {
    return this.inspect();
...
function Od(a, b = {}) {
  ...
  const h = q => {
    ...
    const {type:Vd, index:Wd} = c[f];
    throw new TypeError(`Unexpected ${Vd} at ${Wd}, expected ${q}`);
  }, m = () => {
    let q = "", y;
...
function Qd(a, b = {}) {
  const c = Rd(b), {encode:d = g => g, X:e = !0} = b, f = a.map(g => {
    if ("object" === typeof g) {
...
function Sd(a, b, c) {
  a = a.map(d => Td(d, b, c).source);
  return new RegExp(`(?:${a.join("|")})`, Rd(c));
}
function Ud(a, b, c = {}) {
...
function Td(a, b, c) {
  if (a instanceof RegExp) {
    ...
    Array.isArray(a) ? d = Sd(a, b, c) : d = Ud(Od(a, c), b, c), a = d;
  }
  return a;
}
;const Xd = G("koa-router");
function Y(a, b) {
  a.path && (a.path = b + a.path, a.paramNames = [], b = a.path, a.regexp = Td("string" == typeof b ? b.replace(/\/$/, "") : b, a.paramNames, a.opts));
}
class Yd {
  constructor(a, b, c, d = {}) {
    const {name:e = null} = d;
    ...
    this.path = a;
    this.regexp = Td("string" == typeof a ? a.replace(/\/$/, "") : a, this.paramNames, this.opts);
    Xd("defined route %s %s", this.methods, this.opts.prefix + this.path);
  }
  match(a) {
  ...
  url(a, b) {
    var c = a, d = this.path.replace(/\(\.\*\)/g, ""), e = Pd(d);
...
class Z {
  ...
  register(a, b, c, d = {}) {
    ...
    const l = new Yd(a, b, c, {end:h, name:k, sensitive:m, strict:g, prefix:f, ignoreCaptures:e});
  ...
  match(a, b) {
    const c = this.stack;
    let d;
    const e = {path:[], C:[], route:!1};
    for (let f = c.length, g = 0; g < f; g++) {
      if (d = c[g], Zd("test %s %s", d.path, d.regexp), d.match(a) && (e.path.push(d), 0 == d.methods.length || d.methods.includes(b))) {
        e.C.push(d), d.methods.length && (e.route = !0);
      }
    }
    return e;
  }
...
Z.url = function(a, ...b) {
  return Yd.prototype.url.apply({path:a}, b);
};
const $d = Tb.map(a => a.toLowerCase());
[...$d, "all"].forEach(a => {
  function b(c, d, ...e) {
    "string" == typeof d || d instanceof RegExp || (e = [d, ...e], d = c, c = null);
    this.register(d, "all" == a ? $d : [a], e, {name:c});
    return this;
  }
  Z.prototype[a] = b;
